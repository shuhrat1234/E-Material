...
function AvatarDemo({ lang = 'uz', onBack }) {
  ...
  const startCall = () => {
    ...
    setLastQuestion('');
    setError('');
    playAudio(GREETING_AUDIO, () => {
      // Prompt user or start listening after greeting
    });
  };
    ...
    axios.post(`${API_BASE}/avatar/session/`, { query: q, lang })
      .then(res => {
        ...
        if (audioUrl) {
          ...
          // Fallback to tts endpoint if needed
      ...
      .catch(err => {
        ...
        setError(lang === 'ru' ? 'Xatolik yuz berdi. Iltimos qayta urinib ko\'ring.' : "Xatolik yuz berdi. Iltimos, qayta urinib ko'ring.");
      })
      .finally(() => {
        setLoading(false);
      });
  ...
  return (
    <div className="fixed inset-0 bg-neutral-950 flex flex-col justify-between overflow-hidden select-none font-sans">
      {/* Background Avatar Video (Officer in uniform) */}
      <div className="absolute inset-0 z-0">
        <video
          ref={videoRef}
          src="/officer-idle.mp4"
      ...
      <div className="relative z-20 flex items-center justify-between p-4 sm:p-6">
        ...
        <div className="flex items-center gap-2 px-4 py-1.5 rounded-full bg-black/50 backdrop-blur-md border border-white/15 text-white text-xs font-medium shadow-lg">
          ...
          <span>
            {isSpeaking
              ? (lang === 'ru' ? 'Говорит Jasur (UzbekVoice.ai)' : 'Jasur gapirmoqda (UzbekVoice.ai)')
              : inCall
              ? (lang === 'ru' ? 'На связи • Olmazor IIB' : 'Aloqada • Olmazor IIB')
              : (lang === 'ru' ? 'AI Yordamchi' : 'AI Yordamchi')}
          </span>
        </div>
      ...
      {/* Center Visual Waveform when speaking */}
      {isSpeaking && (
      ...
      <div className="relative z-20 w-full max-w-2xl mx-auto px-4 pb-6 sm:pb-8 flex flex-col items-center">
        {!inCall ? (
          /* Initial Start Call Screen */
          <div className="text-center space-y-4">
            <div className="space-y-1 drop-shadow-md">
              <h2 className="text-xl sm:text-2xl font-bold text-white tracking-tight">
                {lang === 'ru' ? 'AI-помощник следователя' : 'Tergov bo\'limi AI yordamchisi'}
              </h2>
              <p className="text-xs sm:text-sm text-white/80 font-normal">
                {lang === 'ru' ? 'Голос: Jasur (UzbekVoice.ai)' : 'Ovoz: Jasur (UzbekVoice.ai)'}
              </p>
            </div>
            ...
            >
              ...
              <span>{lang === 'ru' ? 'Начать разговор' : "Qo'ng'iroqni boshlash"}</span>
            </button>
          </div>
        ) : (
          /* Active Call Dialog */
          <div className="w-full space-y-3">
            {/* User's question chip */}
            {lastQuestion && (
              ...
              {loading ? (
                <div className="flex items-center gap-2 py-2 text-white/70 text-sm">
                  ...
                  <span className="text-xs text-white/60 ml-2">
                    {lang === 'ru' ? 'Jasur javob tayyorlamoqda...' : 'Jasur ovoz tayyorlamoqda...'}
                  </span>
                </div>
              ) : (
            ...
            <div className="flex items-center gap-2 pt-1 w-full">
              ...
              <form onSubmit={handleSubmit} className="flex-1 min-w-0 flex items-center gap-1.5 bg-white/15 backdrop-blur-xl border border-white/20 rounded-full px-4 py-1.5 shadow-lg">
                <input
                  ...
                  placeholder={isListening ? "Sizni eshityapman..." : (lang === 'ru' ? 'Savolingizni yozing...' : "Savolingizni yozing yoki mikrofondan gapiring...")}
