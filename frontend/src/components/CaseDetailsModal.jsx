...
function CaseDetailsModal({ caseId, lang, user, onSelectCase, onClose }) {
  ...
  return (
    <Modal onClose={onClose} maxWidth="max-w-xl">
      <div className="p-2.5 sm:p-6 flex flex-col flex-1 min-h-0">

        {/* Header */}
        <div className="flex justify-between items-start border-b border-gov-border pb-2.5 sm:pb-3 shrink-0">
          <div className="space-y-1">
            <div className="flex items-center gap-1.5 sm:gap-2 flex-wrap">
              <span className="text-sm font-bold text-gov-primary font-mono whitespace-nowrap">{caseItem.id}</span>
              {citizenMaterials.length > 0 && (
                <button
                  type="button"
                  onClick={() => setActiveTab('citizenCases')}
                  className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] sm:text-[11px] font-bold bg-amber-500/15 text-amber-800 border border-amber-500/30 hover:bg-amber-500/25 transition-colors"
                ...
                >
                  ...
                  <span>
                    {lang === 'ru'
                      ? `Повторный (${citizenMaterials.length + 1})`
                      : `Takroriy (${citizenMaterials.length + 1})`}
                  </span>
                </button>
              )}
            </div>
        ...
        {/* Content */}
        <div className="flex-1 overflow-y-auto py-3 sm:py-4 pb-6 sm:pb-4 pr-0.5">
          ...
          {activeTab === 'info' ? (
            <div className="space-y-3.5 sm:space-y-4 text-xs">
              <div className="space-y-1">
                ...
                <div className="flex items-start justify-between gap-3 bg-gov-light/45 p-2.5 sm:p-3 border border-gov-border rounded">
                  <p className="text-gov-text leading-relaxed font-medium min-w-0 flex-1 break-words text-xs">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                <div className="col-span-2 sm:col-span-1 space-y-1 border border-gov-border rounded p-2.5 bg-gov-light/45">
                  <p className="text-[10px] font-bold text-gov-muted uppercase tracking-wider">{lang === 'ru' ? 'Исполнитель' : 'Ijrochi'}</p>
                  <p className="text-gov-text font-semibold break-words">{officer ? (lang === 'ru' ? officer.name_ru : officer.name_uz) : (lang === 'ru' ? 'Не назначен' : 'Tayinlanmagan')}</p>
                </div>
                <div className="space-y-1 border border-gov-border rounded p-2.5 bg-gov-light/45">
                  ...
                  <p className="text-gov-text font-semibold font-mono text-[11px]">{formatDate(caseItem.registered_at)}</p>
                </div>
                <div className="space-y-1 border border-gov-border rounded p-2.5 bg-gov-light/45">
                  <p className="text-[10px] font-bold text-gov-muted uppercase tracking-wider">{lang === 'ru' ? 'Срок исполнения' : 'Bajarish muddati'}</p>
                  <p className="text-gov-text font-semibold font-mono text-[11px]">{formatDate(caseItem.deadline)}</p>
                </div>
                <div className="space-y-1 border border-gov-border rounded p-2.5 bg-gov-light/45">
                ...
                <div className="space-y-1 border border-gov-border rounded p-2.5 bg-gov-light/45">
                  ...
                  <p className="text-gov-text font-semibold truncate">{getMaterialTypeLabel(caseItem.material_type, lang)}</p>
                </div>
                <div className="space-y-1 border border-gov-border rounded p-2.5 bg-gov-light/45">
                  <p className="text-[10px] font-bold text-gov-muted uppercase tracking-wider">{lang === 'ru' ? 'Источник' : 'Manba'}</p>
                  <p className="text-gov-text font-semibold truncate">{getSourceLabel(caseItem.source_from, lang)}</p>
                </div>
              </div>
              ...
              {citizenMaterials.length > 0 && (
                <div className="space-y-2 pt-2 border-t border-gov-border">
                  <div className="flex items-center justify-between gap-1.5">
                    <div className="flex items-center gap-1.5 text-xs font-bold text-amber-800 min-w-0">
                      <RepeatIcon className="h-3.5 w-3.5 text-amber-700 shrink-0" />
                      <span className="truncate">
                        {lang === 'ru'
                          ? `Другие обращения (${citizenMaterials.length})`
                          : `Boshqa murojaatlari (${citizenMaterials.length})`}
                      </span>
                    </div>
                    <button
                      type="button"
                      onClick={() => setActiveTab('citizenCases')}
                      className="text-[11px] font-bold text-gov-primary hover:underline flex items-center gap-0.5 shrink-0"
                    >
                  ...
                  <div className="space-y-1.5">
                    {citizenMaterials.slice(0, 3).map(cm => (
                      <div
                        ...
                        className="p-2 sm:p-2.5 bg-gov-light/60 border border-gov-border hover:border-gov-primary/50 rounded-lg cursor-pointer transition-all space-y-1"
                      >
                        <div className="flex items-center justify-between gap-1.5">
                          <div className="flex items-center gap-1.5 min-w-0">
                            <span className="font-bold text-gov-primary text-xs font-mono whitespace-nowrap">{cm.id}</span>
                            <span className="text-[10px] text-gov-muted font-mono whitespace-nowrap hidden min-[360px]:inline">{formatDate(cm.registered_at)}</span>
                          </div>
                          <span className={`px-1.5 py-0.5 border rounded text-[9px] font-semibold leading-none shrink-0 ${getStatusBadge(cm.status)}`}>
                            {getStatusText(cm.status)}
                          </span>
                        </div>
                        <div className="flex items-center justify-between gap-2">
                          <p className="text-[11px] text-gov-text font-medium truncate flex-1 min-w-0" title={lang === 'ru' ? cm.title_ru : cm.title_uz}>
                            {lang === 'ru' ? cm.title_ru : cm.title_uz}
                          </p>
                          <span className="text-[10px] font-bold text-gov-primary shrink-0 hover:underline">
                            {lang === 'ru' ? 'Открыть →' : 'Ochish →'}
                          </span>
                        </div>
                        <div className="min-[360px]:hidden text-[9.5px] text-gov-muted font-mono pt-0.5 border-t border-gov-border/30">
                          {formatDate(cm.registered_at)}
                        </div>
                      </div>
                    ))}
                  </div>
              ...
              {/* Status Change */}
              <div className="space-y-2 pt-2.5 border-t border-gov-border pb-2">
                ...
                <div className="flex flex-col sm:flex-row gap-2 items-stretch sm:items-center">
                  ...
                  <button
                    ...
                    className="px-4 py-2 bg-gov-primary hover:bg-blue-700 text-white text-xs font-semibold rounded transition-colors disabled:opacity-50 shrink-0 text-center"
                  >
