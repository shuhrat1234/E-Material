...
function PillBarChart({ title, data = [], trackHeight = 160 }) {
  ...
  return (
    <div className="bg-gov-surface rounded-2xl shadow-card p-5 text-left h-full flex flex-col">
        ...
        {data.map((d, i) => {
          ...
          return (
            <div key={i} className="flex flex-col items-center flex-1 min-w-0">
              ...
              <div className="w-full max-w-[30px] bg-gov-border/40 rounded-full flex items-end overflow-hidden" style={{ height: trackHeight }}>
                <div
                  className="w-full rounded-full transition-all duration-500"
                  style={{
                    height: `${Math.max(pct, 6)}%`,
                    background: isMax ? 'linear-gradient(180deg, #3b82f6, #93c5fd)' : 'rgb(var(--gov-muted))',
                  }}
                />
              </div>
